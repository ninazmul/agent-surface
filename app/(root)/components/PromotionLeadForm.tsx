...
const PromotionLeadForm = ({
  ...
  // Inside your defaultValues in useForm:
  const form = useForm<z.infer<typeof PromotionLeadFormSchema>>({
    resolver: zodResolver(PromotionLeadFormSchema),
    defaultValues: {
      name: Lead?.name || "",
      email: Lead?.email || "",
      number: Lead?.number || "",
      ...
      discount: Lead?.discount || promotion?.discount || undefined,
      note: Lead?.note || "",
  ...
  const onSubmit = async (values: z.infer<typeof PromotionLeadFormSchema>) => {
        ...
        <section className="bg-gray-50 dark:bg-gray-800 p-4 rounded-xl shadow-sm space-y-6">
          ...
          <div>
            ...
            <div className="flex gap-4 overflow-x-auto pb-2">
              {form.watch("course")?.map((course, index) => (
                <div
...
export default PromotionLeadForm;
