...
const NotificationItem = memo(({ n, onDismiss }: { n: NotificationDTO; onDismiss: (id: string) => void }) => (
  <div className="px-4 py-3 border-b flex gap-2 hover:bg-gray-50">
    <Circle className="w-2 h-2 text-red-500 mt-1" fill="currentColor" />

    <a href={n.route || "/notifications"} className="flex-1 text-sm">
      <div className="font-medium line-clamp-2">{n.title}</div>
      <div className="text-xs text-gray-500">{n.agency} • {n.country}</div>
    </a>

    <Button size="icon" variant="ghost" onClick={() => onDismiss(n._id)}>
      <X className="w-3 h-3" />
    </Button>
  </div>
));
...
export default function NotificationsDropdown() {
  ...
  const notificationSound = useRef<HTMLAudioElement>(new Audio("/notification.mp3"));
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    try {
      ...
      const unread = data.filter(
        (n) => !n.readBy?.some((r) => r.email === userEmail && r.status === "read")
      );

      // Play sound only if new unread notifications
      ...
      const newNotifications = unread.slice(0, 5);
      setNotifications((prev) => {
        if (JSON.stringify(prev) !== JSON.stringify(newNotifications)) return newNotifications;
        return prev;
      });
      setUnreadCount(unread.length);

      // Show first load toast
        ...
        <div className="absolute right-0 mt-2 w-[340px] bg-white text-black rounded-2xl shadow-2xl z-50 border">
          ...
          <div className="p-3 border-b flex justify-between items-center">
            ...
            <div className="flex gap-1">
              ...
              <Button size="icon" variant="ghost" onClick={handleReadAll} disabled={unreadCount === 0 || isReadingAll}>
                <CheckCheck className={`w-4 h-4 ${isReadingAll ? "animate-pulse" : ""}`} />
              </Button>
          ...
          {notifications.length === 0 ? (
            ...
            notifications.map((n) => <NotificationItem key={n._id} n={n} onDismiss={dismissNotification} />)
          )}
