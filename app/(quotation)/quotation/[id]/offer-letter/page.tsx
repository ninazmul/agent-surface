      ...
      {courses.length > 0 ? (
        courses.map((c, idx) => (
          ...
          >
            ...
            <div className="ml-4 space-y-1 text-green-700 font-semibold">
              {c?.campuses?.map((campus, idx) => (
                <div key={idx}>
                  <p>{campus.campus}</p>
                  <p>
                    Morning: €{Number(campus.shifts?.morning?.fee || 0)} (
                    {campus.shifts?.morning?.seats} seats)
                  </p>
                  <p>
                    Afternoon: €{Number(campus.shifts?.afternoon?.fee || 0)} (
                    {campus.shifts?.afternoon?.seats} seats)
                  </p>
                </div>
              ))}
            </div>
